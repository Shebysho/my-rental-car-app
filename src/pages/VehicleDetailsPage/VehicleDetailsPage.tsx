...
const BookingSchema = Yup.object().shape({
  ...
  phoneNumber: Yup.string()
    .matches(/^[+]?[0-9]{10,14}$/, 'Invalid phone number format')
    .required('Phone number is required'),
  bookingDate: Yup.date().min(new Date(new Date().setHours(0,0,0,0)), "Cannot book for a past date or today earlier").required('Booking date is required').typeError('Invalid date format'),
  comment: Yup.string().max(250, 'Comment is too long!'),
});
...
const VehicleDetailsPage = () => {
  ...
  if (isLoading && !vehicle) {
    return (
      <Container sx={{display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 'calc(100vh - 100px)', py: 5 }}>
        <CircularProgress />
      </Container>
    );
  }
  ...
  if (typeof vehicle.rentalConditions === 'string' && vehicle.rentalConditions.length > 0) {
    rentalConditionsDisplayItems = vehicle.rentalConditions.split('\n').map(cond => {
     ...
     rentalConditionsDisplayItems = vehicle.rentalConditions.map(cond => {
        const match = String(cond).match(/(\d+)/);
        if (match) {
            const ageOrYear = match[1];
            return String(cond).replace(ageOrYear, `<span style="color: #3470FF; font-weight: 600;">${ageOrYear}</span>`);
        }
        return String(cond);
    });
  }
    ...
    <Container sx={{ paddingTop: 4, paddingBottom: 8, maxWidth: { xs: '100%', md: 540 } }}>
      <Paper elevation={0} sx={{ padding: {xs: 2, sm: 3, md: '40px'}, borderRadius: '24px', position: 'relative' }}>
        ...
        <Stack direction="row" spacing={0.75} sx={{ color: 'rgba(18, 20, 23, 0.5)', fontSize: '12px', lineHeight: 1.5, mb: '24px' }} flexWrap="wrap" useFlexGap>
          {[...(vehicle.accessories || []), ...(vehicle.functionalities || [])].map((item, index, arr) => (
            <React.Fragment key={`acc-func-${index}`}>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: '8px', mb: '24px' }}>
          {rentalConditionsDisplayItems.map((condition, index) => (
            <Chip 
              key={`cond-${index}`} 
        ...
        <Divider sx={{mb: '24px', borderColor: 'rgba(18, 20, 23, 0.2)' }} /> 
